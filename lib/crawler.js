    ...
    "callback": function(error,result,$) {
      ...
      try {
        ...
        $("a[href]").each(function(index,a) {
          ...
          if(!res[href].referrers) res[href].referrers = [];
          if(res[href].referrers.indexOf(po.url) < 0) res[href].referrers.push(po.url);
        });
        ...
        var contentCallback = function(fn) { // std callback for nonHTML content. It can be extended with fn argument!
          return function(err, result) {
            ...
            if(getDomain(url) !== getDomain(po.url) && !opts.followForeigners) return; // don't follow foraign resources if configured...
