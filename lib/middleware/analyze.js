...
exports.keywords = function(opts){
  var mw = function(worker){
    ...
    var mkJob = function(po, root){
      if(po.contentType.indexOf("text/html")>=0 && po.status==200) {
        var fn = function(cb){
          po.keywords = {meta: {description:{}, keywords: {}, title: {}, body: {}}};
          jsdom.env(
            po.document ? po.document : po.url.absolute,
            [],//[jQueryURL],
            function (errors, window) {
              ...
              tmp = window.document.querySelector('meta[name="description"]') && window.document.querySelector('meta[name="description"]').content || "";
              po.keywords.meta.description = kwv.mkVektor(tmp);
              po.keywords.meta.description.text = tmp;
              
              
              tmp = window.document.querySelector('meta[name="keywords"]') && window.document.querySelector('meta[name="keywords"]').content || "";
              po.keywords.meta.keywords = kwv.mkVektor(tmp);
              po.keywords.meta.keywords.text = tmp;
              
              tmp = window.document.querySelector('title') && window.document.querySelector('title').firstChild.nodeValue || "";
              po.keywords.meta.title = kwv.mkVektor(tmp);
              po.keywords.meta.title.text = tmp;
              
              
              
              for(n=1; n<9; n++ ) {
                po.keywords.meta["h"+n] = []; // TODO: {content: []}??
                window.$('h'+n).each(function(){po.keywords.meta["h"+n].push(window.$(this).text());});
              }
              
              var text = window.$("body").text();//window.document.querySelector('body');//.innerText;
              var lowerText = text.toLowerCase();
              var kw = kwv.mkVektor(text);
