...
exports.fetchDocument = function(opts){
  ...
  var filterFn = opts.filterFn || function(po){
    return po.contentType.indexOf("text/html")>=0;
  };
  
  var mw = function(worker){
    worker.name="Document fetcher";
    
    var mkJob = function(po){
      if(filterFn(po)) {
        var fn = function(cb){
        ...
        worker.addJob(fn, {method: "series"});
      }
      return;
    };
    return mkJob;
          ...
          jsdom.env(
            ...
            function (errors, window) {
              ...
              window.$("img").each(function(){
                if(this.src) {
                  po.imgs[this.src] = {src:this.src, alt:this.alt, title:this.title};
                }
              });
              cb();
            }
        ...
        var fn = function(cb){
          jsdom.env(
            ...
            function (errors, window) {
              ...
              if(!po.tags || !po.tags.vektor || !po.tags.count) po.tags = {vektor: {}, count: {}};
              
              var i=0;
              for (i=0; i<tags.length; i++) {
                po.tags.vektor[tags[i]] = window.$(tags[i]).length;
              }              
              cb();
            }
          );
        ...
        var fn = function(cb){
          ...
          jsdom.env(
            ...
            function (errors, window) {
              ...
              po.keywords.body.wc = text.match(/\S+/g).length;
        ...
        var fn = function(cb){
          w3cjs.validate({
            ...
            callback: function(res){
              ...
              if(false && res.messages && res.messages.length>0) {
                console.log("Got errors in "+po.url.absolute);
                console.log(res);
              }
              cb();
            }
